...
MYSQL_STMT *__mysql_find_word = NULL;
MYSQL_STMT *__mysql_document_count = NULL;
MYSQL_STMT *__mysql_find_document = NULL;
...
unsigned long __mysql_get_document_count() {
    ...
    if (!mysql_util_prepare_stmt(mysql, &__mysql_document_count, "SELECT COUNT(*) FROM DOCUMENT;"))
        fail(EX_IOERR, "Failed to prepare statement");

    unsigned long cnt = 0;
    MYSQL_BIND *r = mysql_util_create_bind(MYSQL_TYPE_LONG, &cnt);

    int ret = mysql_util_fetch_result(mysql, __mysql_document_count, NULL, r);
    if (ret == -1)
        fail(EX_IOERR, "Failed to execute statement");

    mysql_stmt_free_result(__mysql_document_count);
    free(r);
...
struct __mysql_id_list *get_word_ids(const char *word) {
    ...
    int status;
    while (1) {
        status = mysql_stmt_fetch(__mysql_find_word);
    ...
    mysql_stmt_free_result(__mysql_find_word);
    free(r);
    free(p);
...
struct __mysql_id_list *get_document_ids(unsigned long word_id) {
    ...
    int status;
    while (1) {
        status = mysql_stmt_fetch(__mysql_find_document);
        if (status == 1 || status == MYSQL_NO_DATA)
            break;
    ...
    mysql_stmt_free_result(__mysql_find_document);
    free(r);
    free(p);
...
void fulltext_db_disconnect() {
    ...
    if (__mysql_find_word != NULL)
        mysql_stmt_close(__mysql_find_word);
    if (__mysql_document_count != NULL)
        mysql_stmt_close(__mysql_document_count);
    if (__mysql_find_document != NULL)
        mysql_stmt_close(__mysql_find_document);
